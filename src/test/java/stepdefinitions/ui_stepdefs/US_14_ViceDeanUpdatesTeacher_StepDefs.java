...
public class US_14_ViceDeanUpdatesTeacher_StepDefs {
    ...
    public void user_finds_on_teacher_list(String string) {
        ...
        List<WebElement> usernameList = Driver.getDriver().findElements(By.xpath("//div[@class='table-responsive']//td[4]"));
        int tableRows = usernameList.size();
        int rowCount = 0;

        while (tableRows > rowCount) {
            for (WebElement usernameElement : usernameList) {
                System.out.println(usernameElement.getText());
                rowCount++;
            }
            ActionsUtils.actionsDoubleClick(teachersPage.nextPageButton);

            usernameList = Driver.getDriver().findElements(By.xpath("//div[@class='table-responsive']//td[4]"));
            tableRows = usernameList.size();
        }
    }
...
//        while()
//
//            if (usernameElement.getText().equalsIgnoreCase(Teachers.getTeacherUsername("US13_TC01"))){
...
//        while (!isUsernameFound) {
//
//            List<WebElement> tableElements = Driver.getDriver().findElements(By.xpath("//table[@class='table table-striped table-bordered table-hover']"));
//            int numberOfRows = tableElements.size();
//            System.out.println(numberOfRows);
//
//            for (int row = 1; row <= numberOfRows; row++) {
//                String usernameText = Driver.getDriver().findElement(By.xpath("//table[@class='table table-striped table-bordered table-hover']//tr[" + row + "]//td[4]")).getText();
